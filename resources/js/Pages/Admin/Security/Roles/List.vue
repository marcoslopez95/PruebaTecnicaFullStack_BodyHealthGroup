<template>
    <div class="flex justify-between text-right my-3 mr-5">
        ...
        <div>
        <ButtonComponent @click="emit('create')" class="uppercase">{{ $t('commons.button.create') }}</ButtonComponent>
        </div>
    </div>

    <TableComponent :headers="headers" :items="helper.items" with-buttons>
    </TableComponent>
</template>
